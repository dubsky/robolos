...
function testErrorInEarlyCallback() {
  ...
  mm.getModuleInfo('a').registerEarlyCallback(goog.listenerObjects.error('error'));
...
function testErrorInErrback() {
  var mm = getModuleManager({'a': [], 'b': ['a']});
  mm.getModuleInfo('a').registerCallback(goog.listenerObjects.error('error1'));
  mm.getModuleInfo('a').registerErrback(goog.listenerObjects.error('error2'));
