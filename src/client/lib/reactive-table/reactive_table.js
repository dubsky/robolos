...
var setup = function () {
    ...
    context.class = this.data.class || this.data.settings.class || 'ui celled striped selectable fluid table ';
...
Template.reactiveTable.events({
    ...
    'click .reactive-table-columns-dropdown a': function (event) {
        var template = Template.instance();
...
Template.reactiveTable.rendered = function() {
        $('.reactive-table-columns-dropdown .ui.dropdown').dropdown();
        var area=$('#'+this.context.id+' > tbody');
        if(this.data.maxHeight!==undefined) {
            area.get(0).style.maxHeight = this.data.maxHeight + 'px';
            /* for use within modals - there are fade-in/out effects and won't size the table properly */
            $('.reactive-table > tbody').mCustomScrollbar({
                theme: "dark-thick",
        ...
        $('#'+this.context.id+' td').bind('mouseenter', function(){
            var $this = $(this);
