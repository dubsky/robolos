...
CURRENT_GRAPH_SESSION_KEY="CURRENT_GRAPH_SESSION_KEY";
...
class SplineChartClass {
    ...
    draw(firstTime,rebuild) {
        ...
        var width = div.get(0).offsetWidth;// parseInt(canvas.attr('width'));  // it's a string in IE6(+?)
        var height = div.get(0).offsetHeight;// parseInt(canvas.attr('width'));  // it's a string in IE6(+?)

        let canvas;
        if(firstTime) {
            canvas = $('<canvas/>, {width: width,height:height}'); //div.find("canvas:first");
        ...
        if(firstTime) {
            $(window).on('resize',function() {
                width = div.get(0).offsetWidth;
                height = div.get(0).offsetHeight;
                me.canvas.attr('width',width-2);
                me.canvas.attr('height',height-2);
        ...
        Session.set(CURRENT_GRAPH_SESSION_KEY,{x:xs,y:ys});
        ...
        forEachXY(function(x, y, pxX, pxY, i) {
            ...
            handle.css({left: (pxX - handleSize / 2 + 1 +2) + 'px', top: ((height - pxY) - handleSize / 2 + 1 +2) + 'px'}); //+2 for the div border
        });
    }
