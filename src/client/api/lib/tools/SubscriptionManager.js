...
SubscriptionManager = function (options) {
    var self = this;
    ...
    self.options.connection = self.options.connection || Meteor;
...
SubscriptionManager.prototype.subscribe = function() {
    ...
    if(Meteor.isClient) {
        ...
        if(Meteor.subscribe) {
            return self.options.connection.subscribe.apply(self.options.connection, arguments);
        }
    }
};

SubscriptionManager.prototype._addSub = function(args) {
    var self = this;
...
SubscriptionManager.prototype._reRunSubs = function() {
    var self = this;
...
SubscriptionManager.prototype._notifyChanged = function() {
    var self = this;
    if(Deps.currentComputation) {
        setTimeout(function() {
            self.dep.changed();
...
SubscriptionManager.prototype._applyCacheLimit = function () {
    var self = this;
...
SubscriptionManager.prototype._applyExpirations = function() {
    var self = this;
    var newCacheList = [];
...
SubscriptionManager.prototype._registerComputation = function() {
    var self = this;
    var computation = Deps.autorun(function() {
        ...
        var ready = true;
        _.each(self._cacheList, function(sub) {
            sub.ready = self.options.connection.subscribe.apply(self.options.connection, sub.args).ready();
...
SubscriptionManager.prototype._createIdentifier = function(args) {
...
SubscriptionManager.prototype._handleError = function(sub) {
    var args = sub.args;
    ...
    function errorHandlingLogic (err) {
        console.log("Error invoking SubscriptionManager.subscribe(%s): ", sub.identifier , err.reason);
...
SubscriptionManager.prototype.reset = function() {
    var self = this;
