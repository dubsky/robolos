...
MongoUploadServer = {
    ...
    init: function (opts) {
        ...
        if (options.uploadUrl != "/upload/") {
            log.debug("Custom upload url setup to: " + options.uploadUrl);
        }

        //RoutePolicy.declare(options.uploadUrl, 'network');
        WebApp.connectHandlers.use(options.uploadUrl, MongoUploadServer.serve);
    },
...
UploadHandler.prototype.post = function () {
    ...
    form.on('fileBegin', function (name, file) {
        ...
        if(options.uploadToFileSystem)
        {
            ...
            let id=folder+newFileName;
            log.debug('Uploading:'+id);
            var gs = new GridStore(Collections.Uploads.rawDatabase(),id, "w", {});
            Fiber(function() {
            Meteor.wrapAsync(function(callback) {
                gs.writeFile( file.path,function(err,ref) {
                    gs.close(callback);
                });
            })();
                Collections.Uploads.upsert({_id:id},{ $set: { _id: id, name: newFileName, folder: folder, size:file.size } });
            }).run();
        }
...
var checkCreateDirectory = function (dir) {
    ...
    if(/^win/.test(process.platform)) {
        prefix=dir.substring(0,2);
        dir = dir.replace(/([A-Z]:[\\\/]).*?/gi, '')
    }

    var dirParts = dir.split('/');
    var currentDir = prefix+'/';
    ...
    for (var i = 0; i < dirParts.length; i++) {
        ...
        if (!fs.existsSync(currentDir)) {
            ...
            log.info('Created directory: ' + currentDir);
        }
    }
}
